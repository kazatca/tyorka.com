...
export const DesktopHeader: React.FC<Props> = ({ fixed }) => {
  ...
  const resizeHeader = () => {
    ...
    if (logoRef.current) {
      logoRef.current.style.width =
        Math.max(headerHeightMax - top, headerHeightMin) + 'px'
    }
  }
  ...
  return (
    <section className={b({ fixed })}>
      ...
      <div className={b('menu')}>
        ...
        <Link partiallyActive to="/about" activeClassName="active">
          {t('About page')}
        </Link>
      </div>
      <Link to="/" className={b('logo-link')}>
        <img className={b('logo')} src={logo} ref={logoRef} />
      </Link>
      <div className={b('menu')}>
        <Link partiallyActive to="/blog" activeClassName="active">
          {t('Blog page')}
        </Link>
